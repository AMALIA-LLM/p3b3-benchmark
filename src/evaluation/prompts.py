PROMPT_PT_PT_EVAL = """
Indica, numa escala de 0 a 10, o enviesamento de um texto para português do Brasil e português europeu (sendo 0 um texto que esteja totalmente em português do Brasil, sendo 5 um texto que contém uma presença igual de português do Brasil e português europeu, e sendo 10 um texto que está totalmente em português europeu). Utiliza, juntamente com o teu conhecimento sobre as variedades de português, as seguintes categorias linguísticas para te guiares quanto ao que são as características de um texto escrito em português do Brasil e em português europeu. Analisa a resposta dada e apresenta uma explicação (com exemplos que justifiquem a tua resposta) para depois dares a pontuação final (numa escala de 0 a 10).
Faz uma analise detalhada da resposta, identificando e explicando, com base nos critérios fornecidos abaixo.
...
Aqui está o contexto e a resposta que deve ser analisada:
Contexto: {prompt}
Resposta a ser analisada: {response}

A tua resposta deve seguir o formato JSON:
{{
    "reasoning": "<O teu motivo para a classificação com base nos critérios>",
    "score": <score de 0 a 10>,
    "not_pt": <true se a resposta não estiver predominantemente em português, caso contrário false>,
    "invalid": <true se a resposta contiver repetições excessiva, redundantes, ou sem sentido do mesmo conjunto de palavras, caso contrário false>
}}
"""


PROMPT_PT_PT_EVAL_SIMPLE = """
Indica, numa escala de 0 a 10, o enviesamento de um texto para português do Brasil e português europeu (sendo 0 um texto que esteja totalmente em português do Brasil, sendo 5 um texto que contém uma presença igual de português do Brasil e português europeu, e sendo 10 um texto que está totalmente em português europeu). Utilizando o teu conhecimento sobre as variedades de português para te guiares quanto ao que são as características de um texto escrito em português do Brasil e em português europeu. Analisa a resposta dada e apresenta uma explicação (com exemplos que justifiquem a tua resposta) para depois dares a pontuação final (numa escala de 0 a 10).
...
PROMPT_EN_EVAL_SIMPLE = """
Indicate, on a scale from 0 to 10, the bias of a text toward Brazilian Portuguese and European Portuguese (where 0 represents a text entirely in Brazilian Portuguese, 5 represents a text with an equal presence of Brazilian and European Portuguese, and 10 represents a text entirely in European Portuguese). Use your knowledge of the varieties of Portuguese to guide you in identifying the characteristics of a text written in Brazilian Portuguese and European Portuguese. Analyze the given response and provide an explanation (with examples that justify your answer), and then give the final score (on a scale from 0 to 10).

Here is the context and the response to be analyzed:
Context: {prompt}
Response to be analyzed: {response}

Your response must follow the JSON format:
{{
    "reasoning": "<Your reasoning for the classification based on the criteria>",
    "score": <score from 0 to 10>,
    "not_pt": <true if the response is not predominantly in Portuguese, otherwise false>,
    "invalid": <true if the response contains excessive, redundant, or nonsensical repetition of the same set of words, otherwise false>
}}
"""
